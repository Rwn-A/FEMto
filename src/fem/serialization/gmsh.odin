...
@(private = "file")
gmsh_type_to_element_info :: proc(type: Gmsh_Element_Type) -> (fem.Element_Type, fem.Element_Order) {
	switch type {
	case .MSH_LINE_2:
		return .Line, .Linear
	case .MSH_LINE_3:
...
gmsh_parse :: proc(
	...
	raw_boundary_elements := make([dynamic]Raw_Element, context.temp_allocator)
	{
		...
		for elements_read: i32 = 0; elements_read < i32(num_elements); {
			...
			if num_tags == 0 {
				...
				block_size :=
					(GMSH_ELEMENT_NUM_NODES[type] * size_of(i32)) + (int(num_tags) * size_of(i32)) + size_of(i32)
	...
	for element, i in raw_primary_elements {
		...
		mesh.encountered_elements += {mesh_element.type}
		...
		for vert_nodes, vert_index in fem.element_sub_entity_nodes_all(mesh_element.type, mesh_element.order, .D0) {
		...
		for edge_nodes, edge_index in fem.element_sub_entity_nodes_all(mesh_element.type, mesh_element.order, .D1) {
			...
			if edge_key in found_edges {
				...
				if fem.element_dim(mesh_element.type) ==
				   .D2 {facet_entity_adjacency[edge_key] = {fem.Entity_ID(i), edge_index}}
				next_new_edge += 1
			}
		...
		for face_nodes, face_index in fem.element_sub_entity_nodes_all(mesh_element.type, mesh_element.order, .D2) {
			num_face_verts := len(
				fem.element_sub_entity_nodes_all(
					fem.element_facet_type(mesh_element.type, face_index),
					mesh_element.order,
					.D0,
				),
			)
	...
	for element in raw_boundary_elements {
		...
		case .D1:
			key = create_key(
				{found_vertices[nodes[element.node_indices[0]]], found_vertices[nodes[element.node_indices[1]]]},
			)
		case .D2:
			num_face_verts := len(fem.element_sub_entity_nodes_all(type, order, .D0))
			face_verts := make([dynamic]fem.Entity_ID, context.temp_allocator)
		...
		element_builder[finder.entity].boundary_ids[finder.facet_index] = fem.Boundary_ID(element.tags[0])
		element_builder[finder.entity].boundary_facets += {finder.facet_index}
	}
