...
// 	current_step:            int,
// 	current_time:            f64,
// 	u_prev, u_prev_prev:     Vector,
// 	u_dot, u_ddot:           Vector,
// 	u_dot_prev, u_ddot_prev: Vector,
// 	allocator:               mem.Allocator,
// 	schemes:                 [MAX_VARS]Time_Scheme,
// }
...
// // initial conditions are cloned using the provided allocator.
// timestepper_create :: proc(
// 	start, end, dt: f64,
// 	ics: Vector,
// 	schemes: [MAX_VARS]Time_Scheme,
...
// 		dt = dt,
// 		current_time = start,
// 		total_steps = int((end - start) / dt),
// 		u_prev = slice.clone(ics, allocator),
// 		u_prev_prev = make(Vector, n, allocator),
// 		u_dot = make(Vector, n, allocator),
// 		u_ddot = make(Vector, n, allocator),
// 		u_dot_prev = make(Vector, n, allocator),
// 		u_ddot_prev = make(Vector, n, allocator),
// 		schemes = schemes,
// 	}
// }

// timestepper_destroy :: proc(ts: ^Timestepper) {
// 	delete(ts.u_prev, ts.allocator)
// 	delete(ts.u_prev_prev, ts.allocator)
// 	delete(ts.u_dot_prev, ts.allocator)
// 	delete(ts.u_ddot_prev, ts.allocator)
// 	delete(ts.u_dot, ts.allocator)
// 	delete(ts.u_ddot, ts.allocator)
// }

// timestepper_step :: proc(ts: ^Timestepper) -> (Timestep, bool) {
// 	if ts.current_step >= ts.total_steps {return {}, false}
// 	defer {ts.current_step += 1; ts.current_time += ts.dt}
// 	return {ts.current_time, ts.current_step, ts.dt}, true
// }


// // Depends on current iterate, so if non-linear, should be called once per non linear iteration for consistency.
// timestepper_derivatives :: proc(
// 	ts: ^Timestepper,
// 	u_cur: Vector,
// 	sys: System,
...
// 	u_ddot = ts.u_ddot
// 	for var in 0 ..< sys.num_vars {
// 		u_cur_range := system_var_slice(sys, var, u_cur)
// 		u_dot_range := system_var_slice(sys, var, u_dot)
// 		u_ddot_range := system_var_slice(sys, var, u_ddot)
// 		u_prev_range := system_var_slice(sys, var, ts.u_prev)
// 		u_prev_prev_range := system_var_slice(sys, var, ts.u_prev_prev)
// 		u_dot_prev_range := system_var_slice(sys, var, ts.u_dot_prev)
// 		u_ddot_prev_range := system_var_slice(sys, var, ts.u_ddot_prev)
...
// 			axpy(u_prev_range, u_dot_range, -1.0 / ts.dt)
// 		case .BDF2:
// 			// first step falls back to BE because u_prev_prev does not exist yet
// 			if ts.current_step <= 1 {
// 				copy(u_dot_range, u_cur_range)
// 				scal(u_dot_range, 1 / ts.dt)
// 				axpy(u_prev_range, u_dot_range, -1.0 / ts.dt)
// 			} else {
// 				copy(u_dot_range, u_cur_range)
// 				scal(u_dot_range, 3 / (2 * ts.dt))
// 				axpy(u_prev_range, u_dot_range, -4 / (2 * ts.dt))
// 				axpy(u_prev_prev_range, u_dot_range, 1 / (2 * ts.dt))
// 			}
// 		case .Newmark:
// 			beta :: 0.25
// 			gamma :: 0.5
...
// 					(u_cur_range[i] - u_prev_range[i]) / (beta * ts.dt * ts.dt) -
// 					u_dot_prev_range[i] / (beta * ts.dt) -
// 					(1 - 2 * beta) / (2 * beta) * u_ddot_prev_range[i]
// 				u_dot_range[i] =
// 					u_dot_prev_range[i] +
// 					ts.dt * (1 - gamma) * u_ddot_prev_range[i] +
// 					ts.dt * gamma * u_ddot_range[i]
// 			}
// 		}
// 	}
...
// // call after nonlinear convergence to advance timestepper state.
// timestepper_commit :: proc(ts: ^Timestepper, u_converged: Vector, sys: System) {
// 	// ensure ts.u_dot/u_ddot are fresh, should always be true
// 	timestepper_derivatives(ts, u_converged, sys)

// 	copy(ts.u_prev_prev, ts.u_prev)
// 	copy(ts.u_prev, u_converged)
// 	copy(ts.u_dot_prev, ts.u_dot)
// 	copy(ts.u_ddot_prev, ts.u_ddot)
// }
