...
Solver_Config :: struct {
    ...
    linsolve_rel_tol: f64,
    linsolve_max_iter: int,

    non_linear_rel_tol: f64,
    non_linear_max_iter: int,
}

Solve_Result :: struct {
    converged: bool,
    reason: enum{Linear_Solver, Non_Linear_Solver},
    final_resid: f64,
    final_iters: int,
}
...
solve_model :: proc(config: Solver_Config, model: ^Model, mesh: fem.Mesh) -> Solve_Result {
    ...
    for steps_taken := 0; ;steps_taken += 1{
        ...
        for {
            non_lin_iters += 1
            converged, iters, resid := la.sparse_cg_solve(J, du, R, config.linsolve_max_iter, config.linsolve_rel_tol)

            if converged == false {
                return {
                    converged = false,
                    reason = .Linear_Solver,
                    final_resid = resid,
                    final_iters = iters,
                }
            }else{
            ...
            R_norm := la.nrm2(R)
            if R_norm / R0_norm < config.non_linear_rel_tol {
...
field_to_output_field :: proc(layout: fem.Layout, field: fem.Field_Handle, u: la.Block_Vector, name: string) -> fio.Output_Field {
    fd := layout.fds[field]
...
field_coeff :: #force_inline proc(layout: fem.Layout, field: fem.Field_Handle, u: la.Block_Vector, element_id: fem.Entity_ID, dof: int) -> f64 {
    return u.values[fem.layout_global_pos(layout, field, element_id, dof)]
}

field_mark_constraints :: proc(mesh: fem.Mesh, layout: fem.Layout, cm: fem.Constraint_Mask, fh: fem.Field_Handle, constrained_facets: []fem.Boundary_ID) {
