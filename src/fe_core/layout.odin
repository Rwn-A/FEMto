...
local_problem_from_basis :: proc(
	basis_infos: []Basis_Info,
	allocator := context.allocator,
) -> (
	la.Block_Vector,
	la.Block_Dense_Matrix,
		...
		for basis_index in 0 ..< b.arity {
			...
			case .D0:
				global_dof^ = assign_shared_dof(
					&vertex_map,
					{conn[.D0][support.entity_index], support.entity_basis_index},
					&next_global,
				)
			case .D1:
				global_dof^ = assign_shared_dof(
...
create_boundary_dof_map :: proc(
	mesh: Mesh,
	layout: Layout,
	fh: Field_Handle,
	allocator := context.allocator,
) -> (
	bdm: Boundary_DOF_Map,
) {
	context.allocator = allocator
...
constraint_mask_create :: proc(field_sizes: []int, allocator := context.allocator) -> Constraint_Mask {
	...
	return {make([]bool, offsets[n], allocator), offsets}
}
